...
/**
 * @title  B20 Society NFT
 * @notice Self-evolving NFT collection. Each NFT starts at phase 1 and can
 *         be advanced up to phase 10 by burning $SOCIETY tokens.
 *
 *  Supply:        1000 unique NFTs
 *  Phases:        1-10 (minted at phase 1, max is phase 10)
 *  Burn curve:    20K, 30K, ..., 100K $SOCIETY per advance
 *  Mint price:    0.001 ETH (sent directly to royalty receiver)
 *  Per-wallet:    5 NFTs max
 *  Royalty:       5% (ERC-2981)
 *
 *  Architecture: SOCIETY token is set once in the constructor and is
 *  immutable for the lifetime of the contract. No admin functions, no
 *  upgrade hooks — fully decentralized from day one.
 */
...
/// @dev Minimal interface for the B20 SOCIETY token. Standard ERC-20
    ...
    ReentrancyGuard
{
    ...
    /// @notice Price to mint one NFT, paid in ETH.
    ...
    /// @notice B20 SOCIETY token used for phase-advance burns. Set once
    ///         at deployment, immutable.
    IBurnable public immutable society;

    /// @notice Address that receives burned SOCIETY tokens. Using a well-
    ...
    /**
     * @param  _society          Predicted address of the B20 SOCIETY token
    ...
    ) ERC721("B20 Society", "B20S") {
        if (_society == address(0) || _royaltyReceiver == address(0)) {
            revert InvalidAddress();
        }
        society = IBurnable(_society);
    ...
    /**
     * @notice Mint one NFT to the caller. Costs MINT_PRICE in ETH.
     *         Each wallet can mint up to MAX_PER_WALLET NFTs.
    ...
    /**
     * @notice Read the burn cost (in $SOCIETY, with 18 decimals) for the
    ...
    /**
     * @notice Advance an NFT to the next phase by burning $SOCIETY
    ...
    function advancePhase(uint256 tokenId) external nonReentrant {
        ...
        uint256 allowed = society.allowance(msg.sender, address(this));
        if (allowed < cost) {
            revert InsufficientAllowance(cost, allowed);
        }

        // Burn: transferFrom user -> dead address. Tokens are permanently
        // unrecoverable; circulating supply decreases even if totalSupply
        // does not.
        bool ok = society.transferFrom(msg.sender, BURN_ADDRESS, cost);
        if (!ok) revert BurnTransferFailed();
     ...
     *         B20 Society Vercel Edge function, which serves dynamic
     *         ERC-7572 metadata with the NFT's current phase image.
     */
